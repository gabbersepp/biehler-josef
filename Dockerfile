...
RUN --mount=type=secret,id=GITHUB_TOKEN \
    --mount=type=secret,id=DEVTO_TOKEN \
    export GITHUB_TOKEN=$(cat /run/secrets/GITHUB_TOKEN) && \
    export DEVTO_TOKEN=$(cat /run/secrets/DEVTO_TOKEN) && \
    npm run contributions && \
    npm run githubcloud && \
    node src/build/github.js && \
    DEBUG=Eleventy* npm run 11ty 2>&1 | tee out_11ty.txt
